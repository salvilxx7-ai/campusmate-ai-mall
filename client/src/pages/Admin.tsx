...
function AdminContent() {
  ...
  return <div className="mx-auto max-w-6xl space-y-7 pb-10"><AdminReviewPanel />
    ...
    <section id="products" className="rounded-[1.5rem] border border-border bg-card p-5 sm:p-7">
      ...
      <div className="mt-6 overflow-x-auto"><table className="w-full min-w-[680px] text-left text-sm"><thead className="border-y border-border bg-secondary/35 text-xs font-semibold text-muted-foreground"><tr><th className="px-4 py-3">商品</th><th className="px-4 py-3">分类</th><th className="px-4 py-3">价格</th><th className="px-4 py-3">状态</th><th className="px-4 py-3 text-right">操作</th></tr></thead><tbody>{items.map(item => <tr key={item.product.id} className="border-b border-border/70 last:border-b-0"><td className="px-4 py-4 font-medium text-foreground">{item.product.title}</td><td className="px-4 py-4 text-muted-foreground">{item.category.name}</td><td className="px-4 py-4 font-mono font-semibold">{formatPrice(item.product.priceCents)}</td><td className="px-4 py-4"><Badge variant="secondary" className="font-normal">{statusLabel[item.product.status]}</Badge></td><td className="px-4 py-4 text-right">{item.product.status === "pending_review" ? <Button size="sm" className="rounded-lg" disabled={statusMutation.isPending} onClick={() => statusMutation.mutate({ productId: item.product.id, status: "active" })}>审核上架</Button> : item.product.status === "archived" ? <Button size="sm" className="rounded-lg" disabled={statusMutation.isPending} onClick={() => statusMutation.mutate({ productId: item.product.id, status: "active" })}>重新上架</Button> : <Button size="sm" variant="outline" className="rounded-lg" disabled={statusMutation.isPending} onClick={() => statusMutation.mutate({ productId: item.product.id, status: "archived" })}>下架</Button>}</td></tr>)}</tbody></table></div>
    </section>
    ...
    <section className="rounded-[1.5rem] border border-border bg-secondary/30 p-5"><div className="flex gap-3"><Database className="mt-0.5 size-5 text-primary" /><div><h2 className="font-display text-lg font-bold">后台权限保护说明</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">导航入口只用于帮助管理员操作；真实权限始终由服务端 `adminProcedure` 校验。普通用户即使直接请求后台接口，也会因当前会话角色不满足要求而被拒绝。</p></div></div></section>
  </div>;
}
