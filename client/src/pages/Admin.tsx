...
function AdminContent() {
  ...
  const uploadKnowledge = trpc.admin.uploadKnowledgeDocument.useMutation({
    onSuccess: result => {
      toast.success("规则文档已处理", { description: `已生成 ${result.chunkCount} 个可引用分块。` });
      void utils.admin.knowledgeDocuments.invalidate();
  ...
  const handleKnowledgeFile = (file?: File) => {
    ...
    reader.onload = () => {
      ...
      uploadKnowledge.mutate({ fileName: file.name, mimeType: file.name.endsWith(".md") ? "text/markdown" : "text/plain", sourceType: uploadType, base64Content: content });
    };
  ...
  return <div className="mx-auto max-w-6xl space-y-7 pb-10">
    ...
    <section id="knowledge" className="rounded-[1.5rem] border border-border bg-card p-5 sm:p-7"><div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-center"><div className="flex gap-4"><span className="grid size-11 shrink-0 place-items-center rounded-xl bg-secondary text-primary"><BookOpen className="size-5" /></span><div><p className="text-xs font-semibold tracking-[0.14em] text-primary">KNOWLEDGE BASE</p><h2 className="mt-1 font-display text-2xl font-bold tracking-[-0.04em]">规则知识库</h2><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">初始化三份来源明确的演示规则后，系统会将内容分块为可引用的确定性词项向量。也可上传管理员拥有发布权的 .md/.txt 文档；当前演示版不解析 PDF。</p></div></div><Button variant="outline" className="rounded-xl" onClick={() => seedKnowledge.mutate()} disabled={seedKnowledge.isPending}><Upload className="mr-2 size-4" />{seedKnowledge.isPending ? "正在初始化…" : "初始化演示规则"}</Button></div><div className="mt-5 flex flex-col gap-3 rounded-xl border border-dashed border-primary/25 bg-secondary/20 p-4 sm:flex-row sm:items-center"><input ref={uploadInputRef} type="file" accept=".md,.txt,text/plain,text/markdown" className="hidden" onChange={event => handleKnowledgeFile(event.target.files?.[0])} /><select value={uploadType} onChange={event => setUploadType(event.target.value as "policy" | "after_sales" | "faq")} className="h-10 rounded-lg border border-border bg-background px-3 text-sm text-foreground"><option value="policy">交易规则</option><option value="after_sales">售后说明</option><option value="faq">FAQ</option></select><Button className="rounded-xl" onClick={() => uploadInputRef.current?.click()} disabled={uploadKnowledge.isPending}><Upload className="mr-2 size-4" />{uploadKnowledge.isPending ? "正在分块…" : "上传规则文档"}</Button><p className="text-xs text-muted-foreground">文档会存入对象存储，数据库只保存元数据、分块与词项向量。</p></div><div className="mt-6 grid gap-3 md:grid-cols-3">{(knowledgeDocuments.data ?? []).map(document => <div key={document.id} className="rounded-xl border border-border bg-secondary/20 p-4"><p className="font-medium text-foreground">{document.title}</p><p className="mt-2 text-xs text-muted-foreground">状态：{document.processingStatus === "ready" ? "可检索" : document.processingStatus}</p><a className="mt-3 inline-block text-xs font-medium text-primary hover:underline" href={document.sourceUrl} target="_blank" rel="noreferrer">查看来源文件</a></div>)}</div></section>
