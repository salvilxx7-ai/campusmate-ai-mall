...
| 模块 | 已实现能力 |
|---|---|
| 商品商城与发布 | 商品浏览、关键词搜索、分类筛选、详情页与学院编辑风视觉设计；登录用户可上传 1–3 张图片、填写价格与详细描述提交闲置物品审核。 |
| 身份与订单 | 普通用户注册/登录与管理员授权的独立入口、Manus OAuth 会话、模拟下单、仅本人可见的订单列表/详情和越权拒绝审计。 |
| 个人中心 | 当前会话范围内的订单轨迹、待审核/展示中的闲置物品、模拟工单进度、统计摘要与本人资料编辑；未登录或其他用户均不会获取个人数据。 |
...
| 管理后台 | 仅管理员可见的待审核商品上架/下架、用户角色管理、模拟工单处理队列、演示规则初始化、受控文档上传、规则版本替换/失效和请求内批量索引重建。 |
...
2. 登录后进入“发布闲置物品”，上传 1–3 张 JPEG/PNG/WebP 图片、填写价格和详细描述，提交后在个人中心查看“等待审核”。
3. 管理员在后台点击“审核上架”，再回到公开商品目录确认只有 `active` 商品可被浏览；待审核商品不能通过直接详情 URL 读取。
