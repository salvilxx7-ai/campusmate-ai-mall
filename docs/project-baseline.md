...
|---|---|---|
| 用户体验 | 学院编辑风的校园二手商城；桌面与移动端都能发现商品、订单、AI 客服、个人中心（含本人资料、发布审核和工单进度），以及清晰区分的普通用户注册/登录与管理员授权入口。 | 为什么商城前台用公开浏览，涉及发布、订单与工单时才进入登录边界；为什么管理员入口不等于自助提权。 |
| 业务闭环 | 商品浏览 → 普通用户发布待审核闲置物品 / 模拟下单 → 我的订单、发布审核和工单进度 → AI 查询本人订单 → 转人工模拟工单。 | 每一步产生了什么数据、由谁读取、如何回到下一步。 |
...
用户身份使用 Manus OAuth，不在 CampusMate 数据库中保存密码。普通用户首次授权会通过 OAuth 回调创建默认 `user` 角色档案；管理员入口只携带白名单站内回跳目标 `/admin`，不能改变角色。回调仍先验证一次性 nonce，再仅允许 `/`、`/profile` 或 `/admin` 三个站内目标；管理员权限始终由服务端 `role` 与 `adminProcedure` 决定。
...
| 用户发布 | 普通用户通过受保护 tRPC 过程提交分类、标题、价格、成色、描述及 1–3 张 JPEG/PNG/WebP 图片；Node 校验后写对象存储引用、商品/图片元数据和审计，初始状态为 `pending_review`。 | 不含支付、真实交易、内容审核模型或恶意文件扫描；待审核物品不能被公开目录或公开详情读取。 |
...
| 发布闲置 | 登录用户在 `/publish` 填写分类、标题、价格、成色、详细描述并上传 1–3 张图片。 | 服务端验证 MIME/大小/字段，写入对象存储引用与 `productImages`，以会话用户写入 `sellerUserId` 和 `pending_review` 商品，并追加发布审计。 | 不接受 GIF/SVG/超 2MB 图片；新商品不进入 `active` 公共目录或公开详情，只有管理员可审核上架。 |
